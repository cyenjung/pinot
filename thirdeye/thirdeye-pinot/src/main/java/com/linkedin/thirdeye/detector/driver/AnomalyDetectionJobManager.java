...
public class AnomalyDetectionJobManager {
  ...
  public void simulatePeriod(Long id, String startIsoString, String endIsoString) throws Exception {
    ...
    BufferedWriter simulationsLogWriter = null;
    BufferedWriter individualExecutionsLogWriter = null;
    try {
      simulationsLogWriter = new BufferedWriter(new FileWriter(
          "/home/jteoh/workspace/thirdeye-anomaly-detector_trunk/thirdeye-anomaly-detector-impl/src/main/resources/sitespeed_thirdeye_hourly_hourly/testing/detection-simulations.log",
          true));

      individualExecutionsLogWriter = new BufferedWriter(new FileWriter(
          "/home/jteoh/workspace/thirdeye-anomaly-detector_trunk/thirdeye-anomaly-detector-impl/src/main/resources/sitespeed_thirdeye_hourly_hourly/testing/detection-individual_executions.log",
          true));
      String functionName = spec.getFunctionName();
      writeExecutionRecord(simulationsLogWriter, id, functionName, startTime, endTime);
      while (currentMonitoringWindowEnd.isBefore(endTime)
        ...
        writeExecutionRecord(individualExecutionsLogWriter, id, functionName,
            currentMonitoringWindowStart, currentMonitoringWindowEnd);
        try {
          String triggerKey = String.format("simulate_period_anomaly_function_trigger_%d_%s-%s",
              spec.getId(), currentMonitoringWindowStart, currentMonitoringWindowEnd);
          Trigger trigger = TriggerBuilder.newTrigger().withIdentity(triggerKey).startNow().build();

          String jobKey = String.format("simulate_period_anomaly_function_job_%d_%s-%s",
              spec.getId(), currentMonitoringWindowStart, currentMonitoringWindowEnd);
          ...
          LOG.error("Error encountered for {}, {}, {}:\n {}", id, currentMonitoringWindowStart,
              currentMonitoringWindowEnd, e);
        }
      ...
      IOUtils.closeQuietly(simulationsLogWriter);
      IOUtils.closeQuietly(individualExecutionsLogWriter);
    }
  }
  ...
  private void writeExecutionRecord(BufferedWriter bw, Long id, String functionName, DateTime start,
      DateTime end) {
    DateTime now = DateTime.now();
    try {
      bw.newLine();
      bw.write(String.format("%s\t%s\t%s\t%s\t%s", id, functionName, start, end, now));
    } catch (IOException e) {
      LOG.error("Unable to write log record: {}\t{}\t{}\t{}", id, functionName, start, end, now);
    }
  }
